...
<div class="page-wrapper">
  <main class="main-content">
    ...
    <div class="board-breadcrumb">
      ...
      <a href="<%= contextPath %>/board/free">자유게시판</a>
      <span class="bc-sep">›</span>
      <span>글 보기</span>
    </div>
    ...
    <div class="content-inner">
      ...
      <div class="view-card">
        ...
        <div class="view-footer">
          <a href="<%= contextPath %>/board/free?page=<%= currentPage %><%= filterParams %>" class="btn-back-list">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
            목록으로
          </a>
          <% if (post.getMemberNo() == loginMemberNo && loginMemberNo > 0) { %>
          <a href="<%= contextPath %>/board/free/edit/<%= post.getPostNo() %>" class="btn-edit-post">
      ...
      <div class="comment-section">
        ...
        <div class="comment-login-notice">
          🔐 <a href="<%= contextPath %>/member/login?redirect=/board/free/view/<%= viewPostNo %>">로그인</a> 후 댓글을 작성할 수 있습니다.
        </div>
        <% } %>
      ...
      <div class="write-bottom-list">
          ...
          <table class="board-table">
            ...
            <tbody>
              ...
              <tr class="<%= isCurrent ? "current-row" : "" %>">
                ...
                <td class="col-title">
                  <a class="post-title-link" href="<%= contextPath %>/board/free/view/<%= lp.getPostNo() %>?page=<%= currentPage %><%= filterParams %>">
        ...
        <nav class="board-pagination">
          ...
          <% } else { %>
            <a href="<%= contextPath %>/board/free/view/<%= viewPostNo %>?page=<%= currentPage - 1 %><%= filterParams %>" class="pg-btn">‹</a>
          <% } %>
          <% if (startPage > 1) { %>
            <a href="<%= contextPath %>/board/free/view/<%= viewPostNo %>?page=1<%= filterParams %>" class="pg-btn">1</a>
          ...
          <% for (int i = startPage; i <= endPage; i++) { %>
            ...
            <% } else { %>
              <a href="<%= contextPath %>/board/free/view/<%= viewPostNo %>?page=<%= i %><%= filterParams %>" class="pg-btn"><%= i %></a>
            <% } %>
          <% } %>
          <% if (endPage < totalPages) { %>
            <% if (endPage < totalPages - 1) { %><span class="pg-ellipsis">···</span><% } %>
            <a href="<%= contextPath %>/board/free/view/<%= viewPostNo %>?page=<%= totalPages %><%= filterParams %>" class="pg-btn"><%= totalPages %></a>
          <% } %>
...
function submitComment() {
  ...
  fetch(ctx + '/board/free/comment/write', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
function deleteComment(commentNo) {
  ...
  fetch(ctx + '/board/free/comment/delete', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
