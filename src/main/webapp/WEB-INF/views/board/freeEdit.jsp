    ...
    <div class="content-inner">
      ...
      <div class="board-header">
        <div class="board-header-left">
          <h1 class="board-title">✏️ 글수정</h1>
        </div>
      </div>

      <% if (post != null) { %>
      <div class="write-card">
        <form class="write-form" action="<%= contextPath %>/board/free/edit/<%= post.getPostNo() %>" method="post"
          ...
          <div class="write-actions">
            <a href="<%= contextPath %>/board/free/view/<%= post.getPostNo() %>" class="btn-cancel">취소</a>
      ...
      <div class="write-bottom-list">
          ...
          <table class="board-table">
            ...
            <tbody>
              ...
              <tr class="<%= isCurrent ? "current-row" : "" %>">
                ...
                <td class="col-title">
                  <a class="post-title-link" href="<%= contextPath %>/board/free/view/<%= lp.getPostNo() %>">
        ...
        <nav class="board-pagination">
          ...
          <% } else { %>
            <a href="<%= contextPath %>/board/free/edit/<%= editPostNo %>?page=<%= currentPage - 1 %>" class="pg-btn">‹</a>
          <% } %>
          <% if (startPage > 1) { %>
            <a href="<%= contextPath %>/board/free/edit/<%= editPostNo %>?page=1" class="pg-btn">1</a>
          ...
          <% for (int i = startPage; i <= endPage; i++) { %>
            ...
            <% } else { %>
              <a href="<%= contextPath %>/board/free/edit/<%= editPostNo %>?page=<%= i %>" class="pg-btn"><%= i %></a>
            <% } %>
          <% } %>
          <% if (endPage < totalPages) { %>
            <% if (endPage < totalPages - 1) { %><span class="pg-ellipsis">···</span><% } %>
            <a href="<%= contextPath %>/board/free/edit/<%= editPostNo %>?page=<%= totalPages %>" class="pg-btn"><%= totalPages %></a>
          <% } %>
